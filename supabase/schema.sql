...
-- público (tipo Google), aprobación por el admin, y contador de clics. Sin
-- verificación de identidad (no tan sensible como el registro de
-- proveedor de servicio) y sin login por ahora.
-- ─────────────────────────────────────────────────────────
...
-- Un negocio "pendiente" no es visible para el rol público bajo las
-- políticas de "businesses" (solo se ven los aprobados), así que no se
-- puede usar una subconsulta normal aquí — se necesita una función segura
-- que sí pueda confirmar su existencia sin depender de esas políticas.
...
-- La subida en sí necesita poder "leer de vuelta" el archivo (mismo patrón
-- que las fotos de proveedor), aunque el bucket ya sea público para lectura
-- vía URL directa.
